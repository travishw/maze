...
func _on_tile_map_done(steps):
	#resets the position in the array
	var n = 0
	#clears the gridmap on subsequent generations
	for i in range(global.width):
		for j in range(global.height):
			set_cell_item(Vector3i(i, 0, j), -1)
			
	#goes throught the array containing the maze building instructions and 
	#places the correct pieces on the grid map
	for i in range(global.width):
		for j in range(global.height):
			if steps[n] == "black":
				pass
			...
			#moves position in array up after each placed piece
			n += 1
